;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;	SOKOBAN GAME JUMP LEFT RIGHT MOD
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;	Jump over an obstable WE the robot
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defrule J_right
	(board ?maxX ?maxY)
	...
	(max-depth ?md)
	(test (< ?n ?md))
	(test (member (create$ block (+ ?x 1) ?y) $?ob))
	(test (not (member (create$ block (+ ?x 2) ?y) $?ob)))
	(test (not (member (create$ box (+ ?x 2) ?y) $?b)))
	(test (not (member (create$ warehouse (+ ?x 2) ?y) $?w)))
	(test (< (+ ?x 1) ?maxX))	
	=>
	(assert (level (+ ?n 1) robot (+ ?x 2) ?y robotf boxes $?b warehouses $?w))
